...
"""
	...
	Special edition for oob routers and direct port connection.

	Syntax: do-oob-device.py device script

	do-oob-device-direct.py imports pwdecrypt.py, which in turn requires environment variables
...
def connect(hostname, username, password, line):
	...
	ssh.connect(hostname, port=line, username=username, password=password)
...
def do_oob_device(device, logfile):
	...
	if logfile == '':
		logfile = device + '-direct-' + os.path.basename(script) + '.log'
